...
##  storing Act_ID as a factor
mean.std.subSet$Act_ID <- as.factor(mean.std.subSet$Act_ID)

cols <- c(names(mean.std.subSet))
##  dim(mean.std.subSet)
##  [1] 10299    88

##  melting the dataset to obtain the mean per subject per activity 
meltingSet <- melt(mean.std.subSet,
                   id.vars=c("Sub_ID", "Act_ID"),
                   measure.vars=cols[3:88], 
...
AveragePerSubjectPerActivity <- dcast(meltingSet,
                                       Sub_ID + Act_ID ~ variable,
                                       mean)

AveragePerSubjectPerActivity <- AveragePerSubjectPerActivity[with(AveragePerSubjectPerActivity, order(Act_ID,Sub_ID)),]
